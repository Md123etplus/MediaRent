...
<div class="premium-upgrade-flow">
    <!-- Payment Step (visible by default for reservation) -->
    <div class="payment-container" id="paymentStep">
        ...
        <div class="payment-header">
            <div class="payment-progress">
                <div class="step active">1. Réservation</div>
            ...
            <div class="selected-plan">
                ...
                <div class="plan-details">
                    <p><i class="fas fa-calendar-alt"></i> Du {{ $reservation['date_debut']->format('d/m/Y') }} au {{ $reservation['date_fin']->format('d/m/Y') }}</p>
                    <p><i class="fas fa-clock"></i> 
                        {{ \Carbon\Carbon::parse($reservation['date_debut'])->diffInDays(\Carbon\Carbon::parse($reservation['date_fin'])) }} jours
                    </p>
                </div>
            </div>
        </div>

        <form method="POST" action="{{ route('reservations.process-payment', $annonce) }}" id="paymentForm">
            @csrf
            <input type="hidden" name="date_debut" value="{{ $reservation['date_debut']->toDateString() }}">
            <input type="hidden" name="date_fin" value="{{ $reservation['date_fin']->toDateString() }}">
                ...
                <div class="order-summary">
                    ...
                    <div class="summary-details">
                        <div class="summary-item">
                            <span>Article :</span>
                            <span>{{ $annonce->objet->nom }}</span>
                        </div>
                        <div class="summary-item">
                            <span>Période :</span>
                            <span>{{ $reservation['date_debut']->format('d/m/Y') }} - {{ $reservation['date_fin']->format('d/m/Y') }}</span>
                        </div>
                        <div class="summary-item">
                        ...
                        <div class="summary-item">
                            <span>Durée :</span>
                            <span>{{ \Carbon\Carbon::parse($reservation['date_debut'])->diffInDays(\Carbon\Carbon::parse($reservation['date_fin'])) }} jours</span>
                        </div>
    ...
    <div class="confirmation-container" id="confirmationStep" style="display: none;">
        <div class="confirmation-content">
            ...
            <p>Votre location a été validée</p>
...
// Script pour le formatage de la carte bancaire (identique à votre version originale)
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (cardNumberInput) {
        cardNumberInput.addEventListener('input', function(e) {
            ...
            if (!isBackspace && formatted.length > cursorPosition && formatted[cursorPosition] === ' ') {
                newCursorPosition++;
            }
    ...
    if (expiryInput) {
        expiryInput.addEventListener('input', function(e) {
            ...
            // Ajuster la position du curseur après le slash
            if (!isBackspace && value.length > 2 && cursorPosition === 2) {
